...
export default function App() {
  ...
  return <Loaded data={state.data} lastMoved={lastMoved} setLastMoved={setLastMoved} />
}
    ...
    <div className="app-shell">
      ...
      <div className="layout">
        <div className="controls-left">
          ...
          />
          <WorkoutPanel workouts={bins.workout_next_day_hrv} />
        </div>
        ...
        <div className="figure-column">
          <div className="figure-wrapper">
            ...
            <div className="headline-anchor">
              <HeadlineNumber hrv={hrv} recovery={recovery} />
            </div>
        ...
        <div className="trajectory-chart">
          {alcohol.trajectory.map((row) => (
            <div className="trajectory-bar" key={row.day}>
              <div
                className={`bar ${row.day === 0 ? 'bar-dip' : ''}`}
                style={{ height: `${row.mean_recovery ?? 0}%` }}
                title={`day ${row.day}: recovery ${row.mean_recovery}%, n=${row.n}`}
              />
              <span className="bar-label">{row.day === 0 ? '0 (drink)' : row.day > 0 ? `+${row.day}` : row.day}</span>
            </div>
          ))}
        </div>
