import { bandColors, bandForHRV, type Theme } from '../lib/bands'
import type { BinsReport } from '../lib/types'

export function WorkoutPanel({ workouts, theme }: { workouts: BinsReport['workout_next_day_hrv']; theme: Theme }) {
    ...
    <section className="workout-panel" aria-label="Workout type and next-day HRV">
      ...
      <ul className="workout-bars">
        {workouts.activities.map((a) => {
          const band = a.mean_next_day_hrv != null ? bandForHRV(a.mean_next_day_hrv) : 'good'
          return (
            <li key={a.activity}>
              ...
              <span className="workout-bar-track">
                <span
                  ...
                  style={{
                    width: `${((a.mean_next_day_hrv ?? 0) / max) * 100}%`,
                    background: colors[band].fill,
                  }}
                />
              </span>
